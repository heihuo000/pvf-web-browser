...
namespace SevenZipSharp
{
        ...
        ICryptoGetTextPassword, IDisposable
    {
            ...
            if (!string.IsNullOrEmpty(fileName))
            {
                ...
                if (baseName.Length > 0 && baseName[baseName.Length - 1] == '.')
                {
                    if (fileName.EndsWith("001") || File.Exists(baseName + "001"))
                    {
                        ...
                        var volName =
                            baseName + (index > 99 ? index.ToString() : index > 9 ? "0" + index : "00" + index);
                        while (File.Exists(volName))
                        {
                            _volumeFileNames.Add(volName);
                            index++;
                            volName = baseName +
                                      (index > 99 ? index.ToString() : index > 9 ? "0" + index : "00" + index);
                        }
                    }
                    else
                    {
        ...
        {
            // Fix when zip file stream opening
            if (_fileInfo == null)
                return -1;
        ...
        public int GetStream(string name, out IInStream inStream)
        {
            ...
            else
            {
                ...
                {
                    var wrapper =
                        new InStreamWrapper(new FileStream(name, FileMode.Open, FileAccess.Read, FileShare.ReadWrite),
                            true);
        ...
        public void Dispose()
        {
            if (_wrappers != null)
            {
                foreach (var wrap in _wrappers.Values)
                {
                    wrap.Dispose();
                }
