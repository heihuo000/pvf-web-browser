...
namespace SevenZipSharp
{
    ...
    internal sealed class LzmaProgressCallback : ICodeProgress
    {
        ...
        public void SetProgress(long inSize, long outSize)
        {
            if (Working != null)
            {
                var newPercentDone = (inSize + 0.0f) / _inSize;
                var delta = newPercentDone - _oldPercentDone;
                if (delta * 100 < 1.0)
                {
                    delta = 0;
                }
                ...
                Working(this,
                    new ProgressEventArgs(PercentDoneEventArgs.ProducePercentDone(newPercentDone),
