...
namespace SevenZipSharp
{
    ...
    public abstract class SevenZipBase : MarshalByRefObject
    {
        ...
        virtual internal void SaveContext(DispatcherPriority priority = DispatcherPriority.Normal)
        {
                    ...
                    if (hresult < -2000000000)
                    {
                        ThrowException(handler,
                            new SevenZipException(
                                "The execution has failed due to the bug in the SevenZipSharp. (code: " +
                                hresult.ToString() + ")"));
                    }
                    else
                    {
                        ThrowException(handler,
                            new SevenZipException(message + " hresult=" +
