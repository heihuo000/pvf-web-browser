...
namespace SevenZipSharp
{
...
#endif
    {
        ...
        private void Init(string archiveFullName)
        {
            ...
            if ((int) _format == -1)
            {
                _format = FileChecker.CheckSignature(archiveFullName, out _offset, out _);
            }
        ...
        private void Init(Stream stream)
        {
            ...
            if ((int) _format == -1)
            {
                _format = FileChecker.CheckSignature(stream, out _offset, out _);
            }
        ...
        public SevenZipExtractor(string archiveFullName, string password) : base(password)
        {
            Init(archiveFullName);
        }
        ...
        /// automatically detects the archive format.</param>
        public SevenZipExtractor(string archiveFullName, string password, InArchiveFormat format) : base(password)
        {
            _format = format;
            Init(archiveFullName);
        }
        ...
        /// <param name="password">Password for an encrypted archive.</param>
        /// <remarks>The archive format is guessed by the signature.</remarks>
        public SevenZipExtractor(Stream archiveStream, string password) : base(password)
        {
            Init(archiveStream);
        }
        ...
        /// automatically detects the archive format.</param>
        public SevenZipExtractor(Stream archiveStream, string password, InArchiveFormat format) : base(password)
        {
            _format = format;
            Init(archiveStream);
        }
        ...
        public long PackedSize
        {
            get
            {
                ...
                return _packedSize.HasValue ? _packedSize.Value :
                    _fileName != null ? (new FileInfo(_fileName)).Length : -1;
            }
        }
        ...
        private IInStream GetArchiveStream(bool dispose)
        {
            ...
            {
                var zipNextVolumeName = _fileName.Substring(0, _fileName.Length - 2) + "01";
                var altSevenZipNextVolumeName = _fileName.Substring(0, _fileName.Length - 3) + "001";
                var sevenZipMultiVolume = _fileName.EndsWith(".001", StringComparison.OrdinalIgnoreCase);
                var altSevenZipVolumeNaming = File.Exists(altSevenZipNextVolumeName);
                var zipMultiVolume = File.Exists(zipNextVolumeName);
                if (!(sevenZipMultiVolume || altSevenZipVolumeNaming || zipMultiVolume))
                    //|| (_volumeFileNames.Count == 1) // mmmmmmmm
                {
                    _archiveStream =
                        new InStreamWrapper(
                            new ArchiveEmulationStreamProxy(
                ...
                {
                    _archiveStream =
                        new InMultiStreamWrapper(_fileName, altSevenZipVolumeNaming, zipMultiVolume, dispose);
                    ...
                    if (_filesCount != 0)
                    {
                        ...
                        try
                        {
                            ...
                            for (uint i = 0; i < _filesCount; i++)
                            {
                                ...
                                {
                                    ThrowException(null,
                                        new SevenZipArchiveException("probably archive is corrupted."));
                                }
                            }
                            ...
                            for (uint i = 0; i < numProps; i++)
                            {
                                ...
                                {
                                    Debug.WriteLine("An unknown archive property encountered (code " +
            ...
            List<uint> actualIndexes, Dictionary<uint, string> altPaths)
        {
            var aec = string.IsNullOrEmpty(Password)
                ? new ArchiveExtractCallback(_archive, directory, filesCount, PreserveDirectoryStructure, actualIndexes,
                    altPaths, this)
                : new ArchiveExtractCallback(_archive, directory, filesCount, PreserveDirectoryStructure, actualIndexes,
                    altPaths, Password, this);
        ...
        /// Gets the IArchiveExtractCallback callback
        /// </summary>
        /// <param name="streamDictionary">The stream dictionary where extract the files </param>
        /// <param name="filesCount">The number of files to be extracted</param>
        /// <returns>The ArchiveExtractCallback callback</returns>
        private ArchiveExtractCallback GetArchiveExtractCallback(Dictionary<uint, Stream> streamDictionary, int filesCount)
        {
            var aec = string.IsNullOrEmpty(Password)
                ? new ArchiveExtractCallback(_archive, streamDictionary, filesCount, this)
                : new ArchiveExtractCallback(_archive, streamDictionary, filesCount, Password, this);
            ArchiveExtractCallbackCommonInit(aec);
            return aec;
        }
        ...
        public void Dispose()
        {
            ...
            {
                Debug.Assert(false,
                    "SevenZipExtractor instance must not be disposed while making an asynchronous method call.");
                return;
            }
        ...
        public bool Check()
        {
            ...
            try
            {
                ...
                using (var aec = GetArchiveExtractCallback("", (int) _filesCount, null, null))
                {
                    try
                    {
                        CheckedExecute(_archive.Extract(null, uint.MaxValue, 1, aec),
        ...
        //Modified 
        /// <summary>
        /// Unpacks the file to a stream dictionary.
        /// </summary>
        public Dictionary<string, Stream> ExtractToStreamDictionary()
        {
            DisposedCheck();
            InitArchiveFileData(false);
            ClearExceptions();
            var indexes = new List<int>();
            indexes.AddRange(_archiveFileData.Where(x => !x.IsDirectory).Select(x => x.Index));
            ...
            var uindexes = new uint[indexes.Count];
            for (var i = 0; i < indexes.Count; i++) uindexes[i] = (uint) indexes[i];
            var origIndexes = new List<uint>(uindexes);
            origIndexes.Sort();
            uindexes = origIndexes.ToArray();
            if (_isSolid.Value) uindexes = SolidIndexes(uindexes);
            var dic = uindexes.ToDictionary<uint, uint, Stream>(item => item, item => new MemoryStream());

            #endregion

            try
            {
                using (var aec = GetArchiveExtractCallback(dic, uindexes.Length))
                {
                    try
                    {
                        CheckedExecute(_archive.Extract(uindexes, (uint) uindexes.Length, 0, aec),
                            SevenZipExtractionFailedException.DEFAULT_MESSAGE, aec);
                    }
                    finally
                    {
            ...
            foreach (var item in dic) streamDictionary.Add(_archiveFileData[(int) item.Key].FileName, item.Value);
        ...
        public void ExtractFile(string fileName, Stream stream)
        {
            ...
            if (index == -1)
            {
                if (!ThrowException(null,
                    new ArgumentOutOfRangeException("fileName",
        ...
        public void ExtractFile(int index, Stream stream)
        {
            ...
            try
            {
                using (var aec = GetArchiveExtractCallback(stream, (uint) index, indexes.Length))
                {
                    try
                    {
                        CheckedExecute(_archive.Extract(indexes, (uint) indexes.Length, 0, aec),
                            SevenZipExtractionFailedException.DEFAULT_MESSAGE, aec);
                    }
                    finally
                    {
        ...
        public void ExtractFiles(string directory, Dictionary<uint, string> altPaths, params int[] indexes)
        {
            ...
            {
                if (!ThrowException(null,
                    new ArgumentException("The indexes must be more or equal to zero.", "indexes")))
                {
                    return;
                }
            }
            ...
            if (uindexes.Where(i => i >= _filesCount)
                .Any(i => !ThrowException(null,
            ...
            else
            {
                foreach (var archiveFileInfo in ArchiveFileData)
                {
                    ...
                    if (extractFileCallbackArgs.ExtractToStream != null ||
                        extractFileCallbackArgs.ExtractToFile != null)
                    {
                        var callDone = false;
                        try
                        {
                            ...
                            {
                                using (var file = new FileStream(extractFileCallbackArgs.ExtractToFile,
                                    FileMode.CreateNew, FileAccess.ReadWrite, FileShare.ReadWrite, 8192))
                                {
                ...
                using ((archiveStream = GetArchiveStream(true)) as IDisposable)
                {
                    ...
                    try
                    {
                        using (var aec = GetArchiveExtractCallback(directory, (int) _filesCount, null, null))
                        {
                            try
                            {
                                CheckedExecute(_archive.Extract(null, uint.MaxValue, 0, aec),
                                    SevenZipExtractionFailedException.DEFAULT_MESSAGE, aec);
                                OnEvent(ExtractionFinished, EventArgs.Empty, false);
                            }
                            finally
