...
namespace SevenZipSharp
{
        ...
        IDisposable
    {
        ...
        public ArchiveUpdateCallback(FileInfo[] files, string[] altNames, int rootLength, SevenZipCompressor compressor,
        ...
        /// <param name="updateData">           The compression parameters.</param>
        /// <param name="directoryStructure">   Preserve directory structure.</param>
        public ArchiveUpdateCallback(FileInfo[] files, string[] altNames, int rootLength, string password,
            SevenZipCompressor compressor, UpdateData updateData, bool directoryStructure) : base(password)
        {
            Init(files, altNames, rootLength, compressor, updateData, directoryStructure);
        }
        ...
        /// <param name="updateData">The compression parameters.</param>
        /// <param name="directoryStructure">Preserve directory structure.</param>
        public ArchiveUpdateCallback(Stream stream, SevenZipCompressor compressor, UpdateData updateData,
        ...
        public ArchiveUpdateCallback(Stream stream, string password, SevenZipCompressor compressor,
            UpdateData updateData, bool directoryStructure) : base(password)
        {
            Init(stream, compressor, updateData, directoryStructure);
        }
        ...
        /// <param name="updateData">The compression parameters.</param>
        /// <param name="directoryStructure">Preserve directory structure.</param>
        public ArchiveUpdateCallback(Dictionary<string, Stream> streamDict, SevenZipCompressor compressor,
            UpdateData updateData, bool directoryStructure)
        {
        ...
        /// <param name="updateData">The compression parameters.</param>
        /// <param name="directoryStructure">Preserve directory structure.</param>
        public ArchiveUpdateCallback(Dictionary<string, Stream> streamDict, string password,
            SevenZipCompressor compressor, UpdateData updateData, bool directoryStructure) : base(password)
        {
            Init(streamDict, compressor, updateData, directoryStructure);
        }
        ...
        private void Init(FileInfo[] files, string[] altNames, int rootLength, SevenZipCompressor compressor,
            UpdateData updateData, bool directoryStructure)
        {
            _files = files;
        ...
        private void Init(Dictionary<string, Stream> streamDict, SevenZipCompressor compressor, UpdateData updateData,
            bool directoryStructure)
        {
            ...
            {
                var str = _streams[i];
                if (str != null) _bytesCount += str.Length;
                ...
                if (fileStream != null)
                {
                    try
                    {
                        ...
                        if (fileInfo.Exists) _streamFileInfos[i] = fileInfo;
                    }
                    catch
                    {
                        //ignored.
        ...
        public int GetProperty(uint index, ItemPropId propID, ref PropVariant value)
        {
            ...
            else if (_streamFileInfos != null && _streamFileInfos.Length > index) fileInfo = _streamFileInfos[index];
            try
            {
                switch (propID)
                {
                    ...
                    case ItemPropId.Attributes:
                        ...
                        if (_updateData.Mode != InternalCompressionMode.Modify)
                        {
                            ...
                            else if (_files == null)
                            {
                                ...
                                {
                                    value.UInt32Value = (uint) (_streams[index] == null
                    ...
                    case ItemPropId.Extension:
                        ...
                        if (_updateData.Mode != InternalCompressionMode.Modify)
                        {
                            try
                            {
                                val = fileInfo != null ? fileInfo.Extension.Substring(1) :
                                    _entries == null ? "" : Path.GetExtension(_entries[index]);
                                value.Value = Marshal.StringToBSTR(val);
                            }
        ...
        public int GetStream(uint index, out ISequentialInStream inStream)
        {
            ...
            if (_files != null)
            {
                ...
                try
                {
                    var fullName = _files[index].FullName;
                    // mmmmm
                    if (File.Exists(fullName))
                    {
                        _fileStream =
                            new InStreamWrapper(
                                new FileStream(fullName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite), true);
                        if (_compressor.ArchiveFormat == OutArchiveFormat.Zip) _wrappersToDispose.Add(_fileStream);
                    }
                }
                catch (Exception e)
                {
