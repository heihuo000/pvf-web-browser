...
namespace SevenZipSharp
{
    ...
    internal class StreamWrapper : DisposeVariableWrapper, IDisposable
    {
        ...
        protected StreamWrapper(Stream baseStream, string fileName, DateTime time, bool disposeStream) : base(
            disposeStream)
        {
        ...
        /// <param name="baseStream">Worker stream for reading, writing and seeking</param>
        /// <param name="disposeStream">Indicates whether to dispose the baseStream</param>
        protected StreamWrapper(Stream baseStream, bool disposeStream) : base(disposeStream)
        {
            _baseStream = baseStream;
        }
    ...
    internal sealed class OutStreamWrapper : StreamWrapper, ISequentialOutStream, IOutStream
    {
        ...
        /// <param name="disposeStream">Indicates whether to dispose the baseStream</param>
        public OutStreamWrapper(Stream baseStream, string fileName, DateTime time, bool disposeStream) : base(
            baseStream, fileName, time, disposeStream)
        {
        }
        ...
        /// <param name="baseStream">Stream for writing data</param>
        /// <param name="disposeStream">Indicates whether to dispose the baseStream</param>
        public OutStreamWrapper(Stream baseStream, bool disposeStream) : base(baseStream, disposeStream)
        {
        }
    ...
    internal class MultiStreamWrapper : DisposeVariableWrapper, IDisposable
    {
        ...
        protected MultiStreamWrapper(bool dispose, bool zip) : base(dispose)
        {
            MultiVolumeZip = zip;
        }
        ...
        {
            foreach (var number in StreamOffsets.Keys)
            {
                if (StreamOffsets[number].Key <= offset && StreamOffsets[number].Value >= offset)
                {
                    return number;
                }
            }
        ...
        public void Seek(long offset, SeekOrigin seekOrigin, IntPtr newPosition)
        {
            ...
            var delta = Streams[CurrentStream]
                .Seek(absolutePosition - StreamOffsets[CurrentStream].Key, SeekOrigin.Begin);
    ...
    internal sealed class InMultiStreamWrapper : MultiStreamWrapper, ISequentialInStream, IInStream
    {
        ...
        /// <param name="dispose">          Perform Dispose() if requested to.</param>
        public InMultiStreamWrapper(string fileName, bool altVolumeNaming, bool zipMultiVolume, bool dispose) : base(
            dispose, zipMultiVolume)
        {
            string baseName;
        ...
        int getIntInBuffer(byte[] data, int bufferSize, long startPosition, long location)
        {
            unchecked
            {
                var value = getByteInBuffer(data, bufferSize, startPosition, location) |
        ...
        public int Read(byte[] data, uint size)
        {
            ...
            var readCount = Streams[CurrentStream].Read(data, 0, readSize);
            readSize -= readCount;
            Position += readCount;
            while (readCount < (int) size)
            {
                if (CurrentStream == Streams.Count - 1) break;
                CurrentStream++;
                Streams[CurrentStream].Seek(0, SeekOrigin.Begin);
                var count = Streams[CurrentStream].Read(data, readCount, readSize);
                readCount += count;
                readSize -= count;
                Position += count;
            }
            ...
            if (zipFormat)
            {
                if (startPosition <= (StreamLength - 22) && StreamLength <= Position)
                {
                    if (getIntInBuffer(data, readCount, startPosition, StreamLength - 22) == 0x06054B50)
                    {
                        ...
                        foreach (var number in StreamOffsets.Keys)
                        {
                            if (StreamOffsets[number].Key > volumeOffset) volumeOffset = StreamOffsets[number].Key;
                        }

                        unchecked
                        {
                            var offset = getIntInBuffer(data, readCount, startPosition, StreamLength - 6) +
                                         volumeOffset;
    ...
    internal sealed class OutMultiStreamWrapper : MultiStreamWrapper, ISequentialOutStream, IOutStream
    {
        ...
        public OutMultiStreamWrapper(string archiveName, int volumeSize, bool altNames) : base(true, false)
        {
        ...
        private void NewVolumeStream()
        {
            ...
            var p = _archiveName.LastIndexOf(".");
            if (p >= 0 && _altNames && (p + 1) == _archiveName.Length - 3)
            {
                if (CurrentStream == 0)
                    newVolumeName = _archiveName;
                else
                    newVolumeName = _archiveName.Substring(0, p + 1) + (CurrentStream > 9
                                        ? (CurrentStream > 99 ? CurrentStream.ToString() : "0" + CurrentStream)
                                        : "00" + CurrentStream);
            }
            else
