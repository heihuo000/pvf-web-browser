...
export class PvfModel {
  ...
  async open(filePath: string, progress?: Progress) {
    ...
    try {
      const { getEncodingMode } = await import('./config.js');
      const mode = getEncodingMode().toUpperCase();
      if (mode === 'AUTO') {
  ...
  // 更新单个文件的内容
  async updateFile(key: string, bytes: Uint8Array): Promise<void> {
    const file = this.fileList.get(key);
    if (!file) {
  ...
  // 递归获取文件夹中的所有文件
  public getAllFilesInFolder(folderPath: string): string[] {
