/* PVF Web Browser - Built with CodeMirror 6 */
...
var API = {
  ...
  // 保存文件到 PVF
  async saveFile(key, content2, encoding) {
    const response = await fetch(`${API_BASE}/api/save-file`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ key, content: content2, encoding })
    });
    return await response.json();
  },
  // 获取书签列表
  ...
  // 批量提取文件
  async batchExtract(keys2, destPath) {
    const response = await fetch(`${API_BASE}/api/batch-extract`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ keys: keys2, destPath })
    });
    return await response.json();
  },
  // 批量提取文件（别名，用于兼容）
  async batchDownload(keys2, destPath) {
    return await this.batchExtract(keys2, destPath);
  },
  // 批量提取文件并打包成zip
  async batchExtractZip(keys2, destPath) {
...
function downloadFile(filename, content2) {
  try {
...
function updateSelectedCount() {
  const selectedFiles2 = document.querySelectorAll('.file-item input[type="checkbox"]:checked:not([data-is-folder="true"])').length;
  const countSpan = document.getElementById("selectedCount");
  if (countSpan) {
    countSpan.textContent = selectedFiles2;
  }
...
// public/js/file-menu.js
var FileMenuManager = class {
  constructor() {
    this.container = null;
    this.menuItem = null;
    this.files = [];
...
// public/js/file-formatter.js
var FileFormatter = class {
  constructor(currentFile2, onPathClick, onUpdate, namePreviewCache, namePreviewPromises) {
    this.currentFile = currentFile2;
    this.onPathClick = onPathClick;
    this.onUpdate = onUpdate;
    this.namePreviewCache = namePreviewCache || /* @__PURE__ */ new Map();
    this.namePreviewPromises = namePreviewPromises || /* @__PURE__ */ new Map();
  }
  setCurrentFile(file) {
  ...
  async addPathLinks(container) {
    ...
    const walker = document.createTreeWalker(
      ...
      NodeFilter.SHOW_TEXT,
      {
        acceptNode: function(node2) {
          if (node2.textContent && !node2.textContent.includes("<") && (pathRegex.test(node2.textContent) || supportTagDescriptions && /\[[^\]]+\]/.test(node2.textContent))) {
    ...
    textNodes.forEach((textNode) => {
      ...
      if (parent) {
        ...
        if (supportTagDescriptions && tagMatch) {
          ...
          tagSpan.dataset.preserveHighlight = "true";
          ...
          tagPreview.dataset.preserveHighlight = "true";
    ...
    const showTagDescriptions = window.showTagDescriptionsMode !== void 0 ? window.showTagDescriptionsMode : true;
  ...
  async loadNamePreview(path, previewElement) {
    ...
    const promise = (async () => {
      try {
        ...
        const nameMatch = content2.match(/\[name\][ \t]*(?:\r?\n|\r)[ \t]*([^\r\n]+)/i);
        ...
        const firstLineMatch = content2.match(/^(.+?)[\r\n]/);
  ...
  async loadTagDescription(extension, tagName, previewElement) {
    ...
    if (this.namePreviewPromises.has(cacheKey)) {
      const promise2 = this.namePreviewPromises.get(cacheKey);
      const description2 = await promise2;
      if (description2) {
        previewElement.textContent = description2;
        previewElement.style.display = "inline";
      } else {
        previewElement.textContent = "";
        previewElement.style.display = "none";
      ...
      try {
        ...
        if (isValueDescription) {
          ...
          if (response.ok) {
            ...
            description2 = valueDescriptions[tagName] || "";
          }
        } else {
          ...
          if (response.ok) {
            ...
            description2 = tagDescriptions[tagKey] || "";
          }
        }
        this.namePreviewCache.set(cacheKey, description2);
        return description2;
      } catch (error) {
...
function applyBracketValueHighlighting(container, extension = "") {
  if (!container) return;
  const codeLines = container.querySelectorAll(".code-line");
  if (!extension) {
    const codeWithLines = container.querySelector(".code-with-lines");
...
var dialogField2 = /* @__PURE__ */ StateField.define({
  create() {
    return true;
  },
  update(value, tr) {
...
var gotoLine = (view) => {
  ...
  if (!panel) {
    ...
    if (view.state.field(dialogField2, false) == null)
      effects.push(StateEffect.appendConfig.of([dialogField2, baseTheme$13]));
    view.dispatch({ effects });
...
// public/js/pvf-language.js
var isLine330 = false;
var pvfHighlightStyle = HighlightStyle.define([
  // 标签（方括号包围的内容，如 [MOTION], [BASE ANI]）
  { tag: tags.labelName, color: "#4ec9b0", fontWeight: "bold" },
  // 青绿色，加粗
  // 结束标签（如 [/MOTION], [/SUB ANI]）- 使用同样的 labelName 标签，但在 token 返回时会特殊处理
  { tag: tags.tagName, color: "#ce9178", fontWeight: "bold" },
  // 橙红色，加粗
  // 字符串（反引号包围的内容，如 `Potion`）
  { tag: tags.string, color: "#ce9178" },
  // 橙红色
  // 字符串链接（尖括号包围的内容，如 <.str对话>）
  { tag: tags.url, color: "#9cdcfe" },
  // 浅蓝色
  // 数字（如 0, 100, 390）
  ...
  // 绿色，斜体
  // 默认文本
  { tag: tags.standard(tags.variableName), color: "#d4d4d4" },
  // 浅灰色
  // 标识符（变量名）
  ...
  // 浅灰色
  // LST文件特定：ID部分（数字）
  { tag: tags.constant, color: "#d7ba7d", fontWeight: "bold" },
  // 黄色，加粗
  // LST文件特定：路径部分（文件路径）
  { tag: tags.link, color: "#9cdcfe" }
  // 浅蓝色
]);
var pvfLanguage = new StreamLanguage({
  name: "pvf",
  startState: () => ({
    inString: false,
    inStringLink: false,
  ...
  token: (stream, state) => {
    ...
    if (state.inStringLink) {
      if (stream.peek() === "`") {
        stream.next();
        state.inStringLinkQuote = !state.inStringLinkQuote;
        return "url";
    ...
    if (numberMatch) {
      const nextChar2 = stream.peek();
      if (!nextChar2 || !/[0-9a-fA-Fx]/.test(nextChar2)) {
        return "number";
      } else {
...
var pvfLanguageSupport = new LanguageSupport(pvfLanguage, [
  syntaxHighlighting(pvfHighlightStyle)
]);
var pvf_language_default = pvfLanguageSupport;

// public/js/codemirror-viewer.js
var NamePreviewCache = class {
  constructor() {
...
var PathPreviewWidget = class extends WidgetType {
  ...
  async loadName() {
    ...
    if (this.namePreviewCache.hasPromise(this.path)) {
      const promise2 = this.namePreviewCache.getPromise(this.path);
      this.name = await promise2;
      this.isLoading = false;
      return this.name;
    }
  ...
  async fetchName() {
    try {
      ...
      if (nameMatch && nameMatch[1]) {
        const name2 = nameMatch[1].trim().replace(/^["'`]|["'`]$/g, "");
        this.namePreviewCache.set(this.path, name2);
        return name2;
      }
      const commentMatch = content2.match(/--.*?名称\s*[：:]\s*([^\r\n]+)/i);
      if (commentMatch && commentMatch[1]) {
        const name2 = commentMatch[1].trim();
        this.namePreviewCache.set(this.path, name2);
        return name2;
      }
  ...
  toDOM() {
    ...
    content2.className = "name-preview-content";
    if (this.isLoading) {
      content2.textContent = "\u52A0\u8F7D\u4E2D...";
      content2.style.color = "#858585";
    } else if (this.name) {
      content2.textContent = this.name;
    } else {
  ...
  getDecorations(view) {
    ...
    for (const { from, to } of view.visibleRanges) {
      ...
      for (let i = 0; i < text.length; i++) {
        const char = text[i];
        if (char === " ") {
          builder.add(pos + i, pos + i + 1, spaceDeco2);
          count++;
        } else if (char === "	") {
          builder.add(pos + i, pos + i + 1, tabDeco2);
          count++;
        }
      }
    }
...
var whitespaceTheme = EditorView.baseTheme({
  ".cm-show-space": {
    ...
    if (show) {
      extension = [
        highlightSpecialChars({
          render: (code) => {
            let span = document.createElement("span");
            span.textContent = code === 32 ? "\xB7" : code === 9 ? "\u2192" : "\u2022";
...
var showWhitespaceMode = true;
var showTagDescriptionsMode = true;
var currentSearchResults = [];
var searchResultsOffset = 0;
var SEARCH_RESULTS_PAGE_SIZE = 500;
var searchResultsTotal = 0;
var searchResultsHasMore = false;
var searchResultsLoading = false;
...
var fileFormatter = new FileFormatter(null, async (path) => {
...
var fileMenuManager = new FileMenuManager();
var modalManager = new ModalManager();
var openModal = modalManager.register("openModal");
var extractModal = modalManager.register("extractModal");
var saveModal = modalManager.register("saveModal");
var editModal = modalManager.register("editModal");
...
function createTab(key) {
  ...
  const fileName = key.split("/").pop();
  const existingTab = tabs.find((t2) => t2.key === key);
  if (existingTab) {
  ...
  tabs.forEach((t2) => t2.isActive = false);
  tabs.push(tab);
  activeTabId = tabId;
  renderTabs();
  return tabId;
...
function switchToTab(tabId) {
  ...
  if (!tab) return;
  tabs.forEach((t2) => t2.isActive = t2.id === tabId);
  activeTabId = tabId;
  currentFile = tab.key;
...
function closeTab(tabId, event) {
  event.stopPropagation();
  const tabIndex = tabs.findIndex((t2) => t2.id === tabId);
  if (tabIndex === -1) return;
  const tab = tabs[tabIndex];
  const wasActive = tab.isActive;
  const otherTabsWithSameFile = tabs.filter((t2) => t2.key === tab.key && t2.id !== tabId);
...
function renderTabs() {
  ...
  container.innerHTML = tabs.map((tab) => `
        <div class="tab ${tab.isActive ? "active" : ""}" data-tab-id="${tab.id}" onclick="switchToTab('${tab.id}')">
            ...
            <span class="tab-close" onclick="closeTab('${tab.id}', event)">\xD7</span>
        </div>
    `).join("");
}
  ...
  try {
    ...
    if (!searchResultsContainer.dataset.hasScrollListener && searchResultsHasMore) {
      ...
      searchResultsContainer.addEventListener("scroll", () => {
        ...
        if (scrollTop + clientHeight >= scrollHeight - 50 && searchResultsHasMore && !searchResultsLoading) {
...
function renderFileTree(files, currentPath2) {
  if (files.length === 0) {
...
function renderSubTree(files, parentPath, container) {
  ...
  if (selectedFolders.size > 0) {
    container.querySelectorAll(".file-item").forEach((item) => {
      const key = item.dataset.key;
      const parentFolder = Array.from(selectedFolders).find(
        (folder) => key === folder || key.startsWith(folder + "/")
      );
      if (parentFolder) {
        const checkbox = item.querySelector('input[type="checkbox"]');
        if (checkbox) {
...
function init() {
      ...
      if (fileTreeItem) {
        ...
        async function expandAndLocate(path) {
          ...
          for (const part of parts) {
            currentPath2 += (currentPath2 ? "/" : "") + part;
  ...
  document.getElementById("confirmSearchBtn")?.addEventListener("click", async (e) => {
    ...
    const keyword2 = document.getElementById("searchKeywordInput")?.value;
  ...
  document.getElementById("confirmEditBtn")?.addEventListener("click", async () => {
    if (!currentFile) return;
    const content2 = document.getElementById("editFileContent").value;
    try {
      const response = await API.updateFile(currentFile, content2);
      if (response.error) {
        alert("\u4FDD\u5B58\u5931\u8D25: " + response.error);
      } else {
  ...
  document.getElementById("confirmEditBookmarkBtn")?.addEventListener("click", () => {
    ...
    const name2 = document.getElementById("editBookmarkName").value;
    if (!name2) {
      alert("\u8BF7\u8F93\u5165\u522B\u540D");
      return;
    }
    bookmarkManager.update(key, name2);
